...
broadcast_similar(f, ::ArrayConflict, ::Type{ElType}, inds::Indices, As...) where ElType =
    similar(Array{ElType}, inds)
broadcast_similar(f, ::ArrayConflict, ::Type{Bool}, inds::Indices, As...) where ElType =
    similar(BitArray, inds)
...
broadcast_similar(f, ::VectorStyle, ::Type{Bool}, inds::Indices{1}, As...) where ElType =
    similar(BitArray, inds)
broadcast_similar(f, ::MatrixStyle, ::Type{Bool}, inds::Indices{2}, As...) where ElType =
    similar(BitArray, inds)
# end FIXME
