...
@test length(randn(Complex{Float64}, 4, 5)) == 20
...
let B = zeros(Complex{Float64}, 2)
    randn!(MersenneTwister(42), B)
    @test B == [Complex{Float64}(-0.5560268761463861,-0.444383357109696),
                Complex{Float64}(0.027155338009193845,-0.29948409035891055)] * 0.7071067811865475244008
end
...
for rng in ([], [MersenneTwister(0)], [RandomDevice()])
    ...
    cftypes = [Complex{Float16}, Complex{Float32}, Complex{Float64}, ftypes...]
