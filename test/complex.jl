...
@testset "Complex{Float16} arithmetic, PR #10003" begin
    @test Float16(1)+Float16(1)im === Complex{Float16}(1, 1)
    @test Float16(1)-Float16(1)im === Float16(1)+Float16(-1)im === Complex{Float16}(1, -1)
    @test Float16(1)*im === Complex{Float16}(im)
    @test Float16(1)/im === Complex{Float16}(0,-1)
    @test Float16(1)^im === Complex{Float16}(1) === Float16(1)+Float16(0)im
end
...
@testset "expm1 type stability" begin
    ...
    @test x isa Complex{Float64}
    x = @inferred expm1(0.1f0im)
    @test x isa Complex{Float32}
end
