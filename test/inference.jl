...
let f(x) = isdefined(x, :re) ? 1 : ""
    @test Base.return_types(f, (Complex{Float32},)) == Any[Int]
    @test Base.return_types(f, (Complex,)) == Any[Int]
end
let f(x) = isdefined(x, :NonExistentField) ? 1 : ""
    @test Base.return_types(f, (Complex{Float32},)) == Any[String]
    @test Union{Int,String} <: Base.return_types(f, (AbstractArray,))[1]
end
import Core.Inference: isdefined_tfunc
@test isdefined_tfunc(Complex{Float32}, Const(())) === Union{}
@test isdefined_tfunc(Complex{Float32}, Const(1)) === Const(true)
@test isdefined_tfunc(Complex{Float32}, Const(2)) === Const(true)
@test isdefined_tfunc(Complex{Float32}, Const(3)) === Const(false)
@test isdefined_tfunc(Complex{Float32}, Const(0)) === Const(false)
...
test_const_return((x)->isdefined(x, :re), Tuple{Complex{Float64}}, true)
