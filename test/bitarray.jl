...
@testset "Binary arithmetic operators" begin
    ...
    @testset "Number/Matrix" begin
        ...
        @check_bit_operation broadcast(/, ci1, b2) Matrix{Complex{Float64}}
        @check_bit_operation broadcast(/, cu1, b2) Matrix{Complex{Float64}}
        @check_bit_operation broadcast(/, cf1, b2) Matrix{Complex{Float64}}
        ...
        @check_bit_operation broadcast(^, 0.0im, b2) Matrix{Complex{Float64}}
        @check_bit_operation broadcast(^, 1.0im, b2) Matrix{Complex{Float64}}
    ...
    @testset "Matrix/Number" begin
        ...
        @check_bit_operation broadcast(-, b1, ci2)  Matrix{Complex{Int}}
        @check_bit_operation broadcast(*, b1, ci2) Matrix{Complex{Int}}
        @check_bit_operation broadcast(/, b1, ci2) Matrix{Complex{Float64}}

        @check_bit_operation broadcast(+, b1, cu2)  Matrix{Complex{UInt8}}
        @check_bit_operation broadcast(-, b1, cu2)  Matrix{Complex{UInt8}}
        @check_bit_operation broadcast(*, b1, cu2) Matrix{Complex{UInt8}}
        @check_bit_operation broadcast(/, b1, cu2) Matrix{Complex{Float64}}

        @check_bit_operation broadcast(+, b1, cf2)  Matrix{Complex{Float64}}
        @check_bit_operation broadcast(-, b1, cf2)  Matrix{Complex{Float64}}
        @check_bit_operation broadcast(*, b1, cf2) Matrix{Complex{Float64}}
        @check_bit_operation broadcast(/, b1, cf2) Matrix{Complex{Float64}}
        ...
        @check_bit_operation broadcast(^, b1, 1.0)   Matrix{Float64}
        @check_bit_operation broadcast(^, b1, 0.0im) Matrix{Complex{Float64}}
        @check_bit_operation broadcast(^, b1, 0x0*im) Matrix{Complex{Float64}}
        @check_bit_operation broadcast(^, b1, 0im)   Matrix{Complex{Float64}}
        @test_throws DomainError broadcast(^, b1, -1)

        b1 = trues(n1, n2)
        @check_bit_operation broadcast(^, b1, -1.0im) Matrix{Complex{Float64}}
        @check_bit_operation broadcast(^, b1, 1.0im)  Matrix{Complex{Float64}}
        @check_bit_operation broadcast(^, b1, -1im)   Matrix{Complex{Float64}}
        @check_bit_operation broadcast(^, b1, 1im)    Matrix{Complex{Float64}}
        @check_bit_operation broadcast(^, b1, 0x1*im)  Matrix{Complex{Float64}}
    end
end
