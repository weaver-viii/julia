...
for elty1 in (Float32, Float64, BigFloat, Complex{Float32}, Complex{Float64}, Complex{BigFloat}, Int)
    ...
    for (t1, uplo1) in ((UpperTriangular, :U),
        ...
        #exp/log
        if (elty1 == Float64 || elty1 == Complex{Float64}) && (t1 == UpperTriangular || t1 == LowerTriangular)
        ...
        for elty2 in (Float32, Float64, BigFloat, Complex{Float32}, Complex{Float64}, Complex{BigFloat}, Int)
        ...
        for eltyB in (Float32, Float64, BigFloat, Complex{Float32}, Complex{Float64}, Complex{BigFloat})
...
for eltya in (Float32, Float64, Complex{Float32}, Complex{Float64}, BigFloat, Int)
    A = eltya == Int ? rand(1:7, n, n) : convert(Matrix{eltya}, eltya <: Complex ? complex.(Areal, Aimg) : Areal)
    # a2 = eltya == Int ? rand(1:7, n, n) : convert(Matrix{eltya}, eltya <: Complex ? complex.(a2real, a2img) : a2real)
    εa = eps(abs(float(one(eltya))))

    for eltyb in (Float32, Float64, Complex{Float32}, Complex{Float64})
