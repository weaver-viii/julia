...
@testset "core functionality" begin
    ...
    for eltya in (Float32, Float64, Complex{Float32}, Complex{Float64}, BigFloat, Int)
        ...
        #for eltya = Complex{Float32} and eltyb = Int. The E[i,j] had NaN32 elements
        ...
        for eltyb in (Float32, Float64, Complex{Float32}, Complex{Float64}, Int)
...
@testset "issue #13243, unexpected nans in complex cholfact" begin
    ...
    r = factorize(apd)[:U]
    E = abs.(apd - r'*r)
    ε = eps(abs(float(one(Complex{Float32}))))
    n = 10
    for i=1:n, j=1:n
        @test E[i,j] <= (n+1)ε/(1-(n+1)ε)*real(sqrt(apd[i,i]*apd[j,j]))
    end
