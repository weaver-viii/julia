...
@testset "UMFPACK wrappers" begin
    ...
    @testset "Core functionality for $Tv elements" for Tv in (Float64, Complex{Float64})
    ...
    @testset "More tests for complex cases" begin
        Ac0 = complex.(A0,A0)
        for Ti in Base.uniontypes(Base.SparseArrays.UMFPACK.UMFITypes)
            Ac = convert(SparseMatrixCSC{Complex{Float64},Ti}, Ac0)
    ...
    @testset "Rectangular cases" for elty in (Float64, Complex{Float64})
    ...
    @testset "Issue #15099" for (Tin, Tout) in (
            (Complex{Float16}, Complex{Float64}),
