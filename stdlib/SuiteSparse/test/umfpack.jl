...
@testset "UMFPACK wrappers" begin
    ...
    @testset "Core functionality for $Tv elements" for Tv in (Float64, Complex128)
        ...
        for Ti in Base.uniontypes(SuiteSparse.UMFPACK.UMFITypes)
            ...
            @test x === z
            y = similar(x)
            SuiteSparse.Ac_ldiv_B!(y, lua, complex.(b,zeros(b)))
            @test y ≈ x
            ...
            @test A.'*x ≈ b
            x = SuiteSparse.At_ldiv_B!(lua,complex.(b,zeros(b)))
            @test x ≈ float([1:5;])
            y = similar(x)
            SuiteSparse.At_ldiv_B!(y, lua,complex.(b,zeros(b)))
            @test y ≈ x
    ...
    @testset "Test aliasing" begin
        a = rand(5)
        @test_throws ArgumentError SuiteSparse.UMFPACK.solve!(a, lufact(speye(5,5)), a, SuiteSparse.UMFPACK.UMFPACK_A)
        aa = complex(a)
        @test_throws ArgumentError SuiteSparse.UMFPACK.solve!(aa, lufact(complex(speye(5,5))), aa, SuiteSparse.UMFPACK.UMFPACK_A)
    end
